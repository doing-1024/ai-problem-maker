...
export async function generateDataPlan(workspaceId) {
  return withWorkspaceLock(workspaceId, 'data', async () => {
    try {
      ...
      const cleanedGenPy = extractCodeBlock(genPy, 'python') || genPy;
      ensurePythonGeneratorShape(cleanedGenPy);
      assertDataPlanLooksReasonable(plan, cleanedGenPy);
      await writeWorkspaceFile(workspaceId, 'data/gen.py', cleanedGenPy);
...
export async function runDataGenerator(workspaceId) {
  return withWorkspaceLock(workspaceId, 'run', async () => {
    let genPy = await safeRead(workspaceId, 'data/gen.py');
    if (!genPy.trim()) {
...
gen_ok = proc.returncode == 0
if gen_ok:
    # Move any files generated by gen.py (excluding our own dirs/files) into out/
    for item in work.iterdir():
        name = item.name
        if name in ("gen.py", "out", "datas.zip"):
            continue
        if item.is_file():
...
if not gen_ok:
    print("ERROR_BEGIN")
    print(f"gen.py exited with code {proc.returncode}")
    print("ERROR_END")
else:
    print("ZIP_BEGIN")
...
`;  
  ...
  if (genError) {
    const genStdout = extractBetween(result.stdout, 'STDOUT_BEGIN', 'STDOUT_END') || '';
    const genStderr = extractBetween(result.stdout, 'STDERR_BEGIN', 'STDERR_END') || '';
    await fs.rm(workDir, { recursive: true, force: true });
    const error = new Error(
      `gen.py 执行失败: ${genError}. ` +
      `stderr: ${genStderr.slice(0, 300)}`
    );
